...
<main >
    ...
    <div class="mx-auto max-w-7xl py-6 sm:px-6 lg:px-8 ">
      ...
      <?php foreach($answers as $key=>$value) : ?>
        <div class="bg-red-50 w-full p-4 flex justify-between mb-[20px]">
          ...
          <div class="flex">
            ...
            <?php if($user_id==$answers[$key]["user_id"]):?>
              <div class="flex flex-col  justify-around">
                <form method="POST" class="flex" action="/answer">
                    <input type="hidden" name="_method" value="DELETE">
                    <input type="hidden" name="answer_id" value="<?= $answers[$key]["id"]?>">
                    <input type="hidden" name="question_id" value="<?= $question["id"]?>">
                    <button >
                ...
                <form action="/answer/edit" method="GET">
                  <button>
                    <input type="hidden" name="answer_id" value="<?=$answers[$key]["id"] ?>">
                    <input type="hidden" name="question_id" value="<?=$question["id"] ?>">
            ...
            <form class="m-[10px] flex flex-col items-center	" method="POST" action="/answer/rating">
              ...
              <input type="hidden" name="answer_id" value="<?= $answers[$key]["id"]?>">
              <input type="hidden" name="question_id" value="<?= $question["id"]?>">
              <button class="<?=$usersAnswersOpinions[$key]["opinion"]=="plus" ? 'border-2 border-indigo-600': 'bug'?> block" name="opinion" value="plus">
                <svg class="w-[20px]" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512"><path d="M256 80c0-17.7-14.3-32-32-32s-32 14.3-32 32V224H48c-17.7 0-32 14.3-32 32s14.3 32 32 32H192V432c0 17.7 14.3 32 32 32s32-14.3 32-32V288H400c17.7 0 32-14.3 32-32s-14.3-32-32-32H256V80z"/></svg>
              </button>
              <span class="text-center"><?= $answers[$key]["rating"] ?></span>
              <button class="<?=$usersAnswersOpinions[$key]["opinion"]=="minus" ? 'border-2 border-indigo-600': 'bug'?> block" name="opinion" value="minus">
                <svg class="w-[20px]" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512"><path d="M432 256c0 17.7-14.3 32-32 32L48 288c-17.7 0-32-14.3-32-32s14.3-32 32-32l352 0c17.7 0 32 14.3 32 32z"/></svg>
              </button>
            </form>
      ...
      <?php if($user_id !=NULL && !$userAlreadyAnswered && $user_id !=$question["user_id"] ):?>
       <form class="mt-[20px]" action="/answer" method="POST">
          ...
          <p class="text-danger break-words	"><?=$errors["body"]?></p>
